"""Migration script to add execution_notes to calendar_items view.

This migration updates the calendar_items view to include execution_notes
...
def migrate_add_execution_notes_to_calendar_view() -> None:
    """Add execution_notes to calendar_items view payload.

    Updates the calendar_items view to include execution_notes field
    in the payload JSONB for planned sessions.
    """
    logger.info("Starting migration: Add execution_notes to calendar_items view")
    ...
    logger.info("Updating calendar_items view to include execution_notes...")

    with engine.begin() as conn:
        # Drop and recreate view with execution_notes
        conn.execute(text("DROP VIEW IF EXISTS calendar_items"))

        # Recreate view with execution_notes in payload
        if _is_postgresql():
            conn.execute(text("""
            ...
            conn.execute(text("""
                ...
                FROM activities a;
            """))
        logger.info("✓ Updated calendar_items view with execution_notes")

    logger.info(f"Migration complete: calendar_items view updated ({db_type})")


if __name__ == "__main__":
    migrate_add_execution_notes_to_calendar_view()
