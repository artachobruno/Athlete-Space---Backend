...
def _update_existing_workouts_table(conn) -> None:
    """Update existing workouts table with missing columns."""
    ...
    for col_name in ["activity_id", "planned_session_id"]:
        ...
        if result.fetchone() is None:
            ...
            # Safe: col_name is from hardcoded list, not user input
...
def _create_new_workouts_tables(conn) -> None:
    """Create new workouts and workout_steps tables."""
    ...
    with engine.begin() as conn:
        try:
            ...
            if _table_exists(conn, "workouts"):
                _update_existing_workouts_table(conn)
                return

            _create_new_workouts_tables(conn)
