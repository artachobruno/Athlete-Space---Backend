...
async def plan_week(
    ...
    plan_exists = _check_weekly_plan_exists(user_id, athlete_id)
    logger.info(
        "B8: Checking for existing weekly plan",
    ...
    try:
        ...
        logger.info(
            f"Weekly plan generated by pipeline: {total_sessions_generated} sessions across {len(planned_weeks)} weeks "
            f"(sessions per week: {sessions_per_week})",
            week_start=monday.isoformat(),
            week_end=sunday.isoformat(),
            weeks_count=len(planned_weeks),
            total_sessions_generated=total_sessions_generated,
    ...
    logger.info(
        f"planner_v2_entry: Weekly plan generation complete - created={saved_count}, "
        f"updated={persist_result.updated}, skipped={persist_result.skipped}, "
        f"success={persist_result.success}, session_ids={session_ids_count}",
        week_start=monday.isoformat(),
        week_end=sunday.isoformat(),
        persisted_count=saved_count,
        updated_count=persist_result.updated,
        skipped_count=persist_result.skipped,
    ...
    # Log detailed persistence result for debugging
    logger.info(
        "Weekly plan persistence result",
        persist_result={
            "created": persist_result.created,
            "updated": persist_result.updated,
            "skipped": persist_result.skipped,
            "success": persist_result.success,
            "session_ids": persist_result.session_ids[:10] if persist_result.session_ids else [],  # Log first 10 IDs
    ...
    if not persist_result.success or not persist_result.session_ids:
        logger.error(
            ...
            week_start=monday.isoformat(),
            week_end=sunday.isoformat(),
            persist_result_created=persist_result.created,
            persist_result_updated=persist_result.updated,
            persist_result_skipped=persist_result.skipped,
            persist_result_success=persist_result.success,
            session_ids_count=len(persist_result.session_ids) if persist_result.session_ids else 0,
            user_id=user_id,
            athlete_id=athlete_id,
        )
    ...
    # Warn if 0 sessions were created (but persistence succeeded)
    if saved_count == 0:
        logger.warning(
            f"Weekly plan created with 0 sessions - updated={persist_result.updated}, "
            f"skipped={persist_result.skipped}, session_ids={session_ids_count}. "
            f"All sessions may have been skipped or updated instead of created.",
            week_start=monday.isoformat(),
            week_end=sunday.isoformat(),
            updated_count=persist_result.updated,
            skipped_count=persist_result.skipped,
    ...
    if saved_count > 0 and persist_result.updated > 0:
        ...
        sessions_message = f"• **{saved_count} training sessions** added to your calendar"
    elif persist_result.updated > 0:
